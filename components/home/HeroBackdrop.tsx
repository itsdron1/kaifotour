...
export function HeroBackdrop({ image }: { image: PhotoSource }) {
  ...
  return (
    <div className="absolute inset-0 -z-10">
      <motion.div className="absolute inset-0" style={reduceMotion ? { scale: 1.04 } : { y, scale }}>
      ...
      <motion.div aria-hidden="true" className="absolute inset-0 bg-deep" style={{ opacity: reduceMotion ? 0 : shade }} />
    </div>
  );
}
