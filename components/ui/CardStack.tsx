...
export interface CardStackLabels {
  /** Подпись над стопкой и её название для скринридера */
  title: string;
  pause: string;
  play: string;
  prev: string;
...
interface CardStackProps<T extends { id: string }> {
  items: T[];
  /** Вид карточки: expanded — карточка наверху стопки и раскрыта наведением или тапом */
  renderCard: (item: T, state: { expanded: boolean }) => ReactNode;
  /** По этой карточке считается высота стопки: берите самую высокую по содержанию */
  spacerItem?: T;
  labels: CardStackLabels;
  /** Подпись над стопкой: рядом с собственным заголовком раздела она лишняя */
  showTitle?: boolean;
  className?: string;
}

/**
 * Стопка открыток с автоматическим перекладыванием раз в 3 секунды.
...
export function CardStack<T extends { id: string }>({
  ...
  const spacer = spacerItem ?? items[0];
  ...
  useEffect(() => {
    ...
    const leavingId = items[previous]?.id;
    if (!leavingId) return;
  ...
  }, [index, items, reducedMotion, total]);
  ...
  useEffect(() => {
    ...
    const onPointerDown = (event: PointerEvent) => {
      ...
      if (!target?.closest?.("[data-card-stack]")) setHeld(false);
    };
  ...
  // Тач: свайп листает, короткий тап ставит на паузу и раскрывает, ссылки работают обычным тапом
  ...
  return (
    <div className={cn("w-full", className)}>
      <div className={cn("flex items-center gap-4", showTitle ? "justify-between" : "justify-end")}>
        {showTitle ? <p className="kicker text-secondary">{labels.title}</p> : null}
      ...
      <div
        ...
        onClickCapture={(event) => {
          // Тап и свайп по стопке не должны открывать ссылку: её открывает только клик мышью
      ...
      >
        ...
        <div aria-hidden="true" className="invisible">
          {renderCard(spacer, { expanded: false })}
        </div>
        ...
        {items.map((item, cardIndex) => {
          ...
          return (
            <motion.div
              ...
              inert={!isTop}
            >
              {renderCard(item, { expanded: isTop && lifted })}
